...
def get_model(model_name, img_size, channels, num_classes=2, shallow=True, alpha=0.25):
    model = Sequential()
    if(channels == 1):
    ...
    if(model_name == 'mobilenet'):
        model.add(Convolution2D(int(32 * alpha), (3,3), strides=(2,2), padding="same", use_bias=False))
    ...
    if(model_name == 'ds_cnn'):
        model.add(Convolution2D(32, (3,3), strides=(2,2), padding='same', use_bias=False))
        model.add(BatchNormalization())
        model.add(Activation('relu'))
